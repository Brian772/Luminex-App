...
export default function Sidebar({ isOpen }: { isOpen: boolean }) {
  ...
  const getLinkClass = (href: string) =>
    `flex items-center gap-3 py-1.5 pl-3 pr-2 text-sm transition-colors relative whitespace-nowrap ${
      isActive(href)
        ? 'text-[var(--color-accent-primary)] font-medium border-l-2 border-[var(--color-accent-primary)] bg-[var(--color-accent-primary)]/[0.06] rounded-r-md'
    ...
    >
      ...
      <div className="w-64 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto py-2 space-y-5">
          {/* Nav section uses -mx-px so border-l aligns with sidebar border */}
          <div>
