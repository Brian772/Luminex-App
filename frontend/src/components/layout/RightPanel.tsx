...
export default function RightPanel({ isOpen }: { isOpen: boolean }) {
  ...
  return (
    <aside
      className={`border-l border-[var(--color-border-subtle)] bg-[var(--color-bg-primary)] flex-shrink-0 flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden transition-all duration-300 ${
      ...
      <div className="flex border-b border-[var(--color-border-subtle)] px-2 pt-2">
        {(['ringkasan', 'tugas', 'flashcards'] as TabType[]).map((tab) => (
          <button
            ...
            className={`flex-1 pb-2 pt-1 px-1 text-sm font-medium capitalize transition-colors border-b-2 ${
              activeTab === tab 
        ...
        {activeTab === 'ringkasan' && (
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-semibold text-[var(--color-text-primary)] opacity-50 uppercase tracking-wider">Poin Utama</span>
